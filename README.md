...
- **关键链路**：识别 main / HTTP / CLI / 公共 API / 测试入口，沿调用边追踪到数据库、网络、文件系统、消息队列或外部进程
- **链路时序视图**：按执行顺序分入口、内部调用、I/O 边界三条泳道，确定步骤与推断步骤分别标记
...
关键链路同样遵守这个边界：默认只沿 `exact` / `likely` 调用边，`exact` 显示为
“确定”，`likely` 显示为“推断”，不会把静态启发式冒充实际运行轨迹。LLM 只解释
